...
class _DFileUploadComponentState extends State<DFileUploadComponent> {
  ...
  void pickSingleFile() async {
    ...
    if (result != null) {
      ...
      fileCont.text = "${appointmentAppStore.reportList.length} ${languageTranslate('lblFilesSelected')}";
    } else {
      toast(languageTranslate('lblNoReportWasSelected'));
    }
    setState(() {});
  }
      ...
      builder: (_) => Column(
        ...
        children: [
          AppTextField(
            ...
            readOnly: true,
            decoration: textInputStyle(context: context, text: languageTranslate('lblAddMedicalReport')).copyWith(
              suffixIcon: Row(
                  ...
                  return Stack(
                    ...
                    children: [
                      SettingItemWidget(
                        ...
                        title: '${languageTranslate("lblMedicalReport")} ${index + 1}',
                        trailing: IconButton(
                          onPressed: () {
                            launchUrl("${data.url}");
                          },
